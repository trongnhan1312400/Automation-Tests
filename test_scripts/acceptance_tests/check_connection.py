...
class TestScenario03(TestScenarioBase):
    ...
    def __init__(self):
        super().__init__(total_steps=6, test_name="test_scenario_03_check_connection")
    ...
    async def execute_test_case(self):
        ...
        self.steps[0].set_name("Create pool ledger")
        await perform(self.steps[0], pool.create_pool_ledger_config, self.pool_name, pool_config)

        # 2. Create wallet
        self.steps[1].set_name("Create wallet")
        await perform(self.steps[1], wallet.create_wallet, self.pool_name, self.wallet_name, None, None, None)
        self.wallet_handle = await perform(self.steps[2], wallet.open_wallet, self.wallet_name, None, None)

        # 3. Create DID
        self.steps[2].set_name("Create DID")
        ...
        self.steps[3].set_name("Connect to pool")
        self.pool_handle = await perform(self.steps[3], pool.open_pool_ledger, self.pool_name, None)

        # 5. Disconnect from pool.
        self.steps[4].set_name("Disconnect form pool")
        await perform(self.steps[4], pool.close_pool_ledger, self.pool_handle)

        # 6. Reconnect to pool.
        self.steps[5].set_name("Reconnect to pool")
        self.pool_handle = await perform(self.steps[5], pool.open_pool_ledger, self.pool_name, None)
