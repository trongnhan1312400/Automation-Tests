    ...
    async def execute_test_steps(self):
        ...
        claim_req = await \
            utils.perform(self.steps,
                          ...
                          json.dumps(claim_offer), claim_def, secret_name)

        # 8. Create claim.
        self.steps.add_step("Create claim")
        created_claim = await \
            utils.perform(self.steps, anoncreds.issuer_create_claim,
                          self.wallet_handle, json.dumps(claim_req),
                          json.dumps(constant.sample_claim_json), -1)
        ...
        utils.check(self.steps, error_msg,
                    condition=lambda:
                    created_claim['claim'] == claim_offer)
