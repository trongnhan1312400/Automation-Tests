...
class CryptoSetKeyMetadataWithEmptyMetadata(CryptoTestBase):
    async def test(self):
        # 1. Create wallet.
        # 2. Open wallet.
        ...
        # 4. Set empty metadata for created verkey and
        # verify that there is no exception raised.
        self.steps.add_step("Set empty metadata for created verkey and "
                            "verify that there is no exception raised")
        metadata = ""
...
if __name__ == "__main__":
    CryptoSetKeyMetadataWithEmptyMetadata().execute_scenario()
