...
class BuildGetNymRequest(TestScenarioBase):
    ...
    async def execute_test_steps(self):
        ...
        await perform(self.steps, Common().build_and_send_nym_request, self.pool_handle,
                      self.wallet_handle, submitter_did, target_did, target_verkey, None, None)

        # 4. build get nym request
        self.steps.add_step("build GET_NYM request")
        get_nym_req = json.loads(await perform(self.steps, ledger.build_get_nym_request,
                                               submitter_did, target_did))

        # 5. Verifying get_nym_request json correctly.
        self.steps.add_step("Verifying get_nym_request json")
        expected_response = json.loads(JsonTemplate.get_nym_response.format(submitter_did,
                                                                            "105", target_did))
