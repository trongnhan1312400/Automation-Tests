...
from libraries.utils import perform, compare_json, generate_random_string


class GetSchemaRequest(TestScenarioBase):

    async def execute_test_steps(self):
        print("GetSchemaRequest started")
        ...
        await perform(self.steps, ledger.sign_and_submit_request, self.pool_handle,
        ...
        expected_response = json.loads(JsonTemplate.get_schema_response.format(submitter_did, "107",
                                                                               target_did, data_response))

        get_schema_req = json.loads(await perform(self.steps, ledger.build_get_schema_request, submitter_did,
                                                  target_did, data_response))

        # 6. Verify json get schema request is correct.
        self.steps.add_step("Verify json get schema request is correct.")
        try:
            assert compare_json(expected_response, get_schema_req)
        ...
        print("GetSchemaRequest completed")
