...
class BuildNymRequest(TestScenarioBase):
    ...
    async def execute_test_steps(self):
        ...
        (target_did, target_verkey) = await perform(self.steps, signus.create_and_store_my_did, self.wallet_handle,
                                                    json.dumps({"seed": seed_trustee_2}))

        # 3. build nym request
        self.steps.add_step("build NYM request")
        nym_req_txn = json.loads(await perform(self.steps, ledger.build_nym_request, submitter_did, target_did,
                                               target_verkey, None, None))

        # 4. Verifying json nym request response correctly.
        self.steps.add_step("Verifying nym request")
        expected_response = json.loads(JsonTemplate.message.format(0, submitter_did, "1",
        ...
        try:
            ...
            assert nym_req_txn["operation"]["type"] == expected_response["operation"]["type"]
            assert nym_req_txn["operation"]["dest"] == expected_response["operation"]["dest"]
            assert nym_req_txn["operation"]["verkey"] == expected_response["operation"]["verkey"]
            self.steps.get_last_step().set_status(Status.PASSED)
        except AssertionError:
            self.steps.get_last_step().set_message(Message.JSON_INCORRECT.format(""))
        print("BuildNymRequest completed")
