...
class TestDecryptWithValidData(SignusTestBase):
    ...
    async def execute_test_steps(self):
        ...
        (encrypted_message, nonce) = \
            await utils.perform(self.steps, signus.encrypt, self.wallet_handle,
        ...
        decrypted_message = await \
            utils.perform(self.steps, signus.decrypt, self.wallet_handle,
                          self.pool_handle, my_did, their_did,
                          encrypted_message, nonce)

        # 10. Check 'decrypted_message'
        self.steps.add_step("Check 'decrypted_message'")
        error_message = "'decrypted_message' mismatches with 'message'"
