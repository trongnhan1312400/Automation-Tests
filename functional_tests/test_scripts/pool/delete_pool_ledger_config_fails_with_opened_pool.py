...
class TestOpenPoolLedgerConfig(PoolTestBase):
    ...
    async def execute_test_steps(self):
        ...
        # 3. Delete opened pool ledger and verify that a opened pool ledger cannot be deleted.
        self.steps.add_step("Delete opened pool ledger and verify that a opened pool ledger cannot be deleted")
        await utils.perform_with_expected_code(self.steps, pool.delete_pool_ledger_config, self.pool_name,
                                               expected_code=ErrorCode.CommonInvalidState)
        # Check pool config folder still exists.
        utils.check(self.steps, error_message="Pool config folder still exists",
